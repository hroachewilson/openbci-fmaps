...
http://mne-tools.github.io/dev/auto_examples/decoding/plot_decoding_csp_eeg.html
...
raw_fnames = eegbci.load_data(subject, runs, path='/home/harry/eeg/eegmmidb/S001')

raw = concatenate_raws([read_raw_edf(f, preload=True, stim_channel='auto') for f in
             raw_fnames])
...
scores = cross_val_score(Pipeline([('CSP', csp), ('LDA', lda)]),
                         epochs_data_train, labels, cv=cv, n_jobs=1)
